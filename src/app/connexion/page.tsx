...
export default function ConnexionPage() {
  ...
  useEffect(() => { document.title = 'Connexion - Mosquée Bilal'; }, []);
  ...
  const [visiteurForm, setVisiteurForm] = useState({ firstname: '', lastname: '', email: '', phone: '', address: '', newsletter: false });
  ...
  // Redirige selon le role apres connexion (sauf si ?next=... fourni)
  const redirectAfterLogin = async (userId: string) => {
    const next = searchParams.get('next');
    if (next && next.startsWith('/')) { router.push(next); return; }
    const { data: profile } = await supabase.from('profiles').select('role').eq('id', userId).single();
    const role = profile?.role ?? 'visiteur';
    if (role === 'visiteur') router.push('/mon-profil');
    else if (role === 'gestionnaire_obseques') router.push('/admin/dashboard/obseques');
  ...
  const handleVisiteurSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    await supabase.from('demandes_acces').insert({
      ...
      newsletter_opt_in: visiteurForm.newsletter,
    });
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-serif text-primary mb-2">Connexion</h1>
        </div>
              ...
              {visiteurSubmitted ? (
                ...
                <form onSubmit={handleVisiteurSubmit} className="space-y-4">
                  ...
                  <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl bg-surface-container-low hover:bg-surface-container transition-colors">
                    <input
                      type="checkbox"
                      checked={visiteurForm.newsletter}
                      onChange={(e) => setVisiteurForm({ ...visiteurForm, newsletter: e.target.checked })}
